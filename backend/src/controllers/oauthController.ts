...
export const initiateGoogleOAuth = asyncHandler(async (req: FastifyRequest, reply: FastifyReply) => {
  if (!req.userId) {
...
export const handleGoogleOAuthCallback = asyncHandler(async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  try {
    ...
    console.error('[OAuthController] Error exchanging code or saving tokens:', error);
     // --- CORRECTED REDIRECT (using FRONTEND_ORIGIN) ---
    reply.redirect(`${frontendOrigin}/dashboard?error=${encodeURIComponent(error.message || 'token_exchange_failed')}`);
  }
});

// 3. Disconnect Google Account
export const disconnectGoogleAccount = asyncHandler(async (req: FastifyRequest, reply: FastifyReply) => {
